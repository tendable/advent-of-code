...
void Matrix::drawLine(Point a, Point b) {
    floorY = std::max(floorY, std::max(a.y, b.y));
...
void Matrix::startDroppingSand() {
    ...
    std::cout<< "Sand: " << amountOfSandDropped + 1 << std::endl;
}

Point Matrix::nextPointForSand(Point currentLocation) {
    if (m_Matrix[currentLocation.y][currentLocation.x] == 'o') return SandPoint;
    // Try to move down
    {
...
bool Matrix::dropSand() {
    Point dropPoint = SandPoint;
    Point lastPoint = dropPoint;
    Point nextPoint = dropPoint;

    do {
        lastPoint = nextPoint;
        nextPoint = nextPointForSand(nextPoint);
        if (nextPoint == dropPoint) return false;
